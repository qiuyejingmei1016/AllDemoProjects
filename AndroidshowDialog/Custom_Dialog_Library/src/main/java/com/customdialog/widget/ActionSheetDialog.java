package com.customdialog.widget;
...
public class ActionSheetDialog {
    ...
    public ActionSheetDialog addSheetItem(List<String> list, SheetItemColor color,
                                          OnSheetItemClickListener listener) {
        if (sheetItemList == null) {
            sheetItemList = new ArrayList<SheetItem>();
        }
        for (String content : list) {
            sheetItemList.add(new SheetItem(content, color, listener));
        }
    ...
    private void setSheetItems() {
        ...
        for (int i = 1; i <= size; i++) {
            ...
            if (size == 1) {
                if (showTitle) {
                    textView.setBackgroundResource(R.drawable.dialog_com_bg_bottom_btn_selector);
                } else {
                    textView.setBackgroundResource(R.drawable.dialog_com_white_bg);
                }
            } else {
                if (showTitle) {
                    if (i >= 1 && i < size) {
                        textView.setBackgroundResource(R.drawable.dialog_com_bg_item_selector);
                    } else {
                        textView.setBackgroundResource(R.drawable.dialog_com_bg_bottom_btn_selector);
                    }
                } else {
                    if (i == 1) {
                        textView.setBackgroundResource(R.drawable.dialog_com_bg_top_btn_selector);
                    } else if (i < size) {
                        textView.setBackgroundResource(R.drawable.dialog_com_bg_item_selector);
                    } else {
                        textView.setBackgroundResource(R.drawable.dialog_com_bg_bottom_btn_selector);
