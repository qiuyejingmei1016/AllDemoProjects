...
public class CustomInputFilter implements InputFilter {
    ...
    public CustomInputFilter(Context context) {
    ...
    public CharSequence filter(CharSequence source, int start, int end, Spanned dest, int dstart, int dend) {
        ...
        if (emojiMatcher.find()) {
            UIAction.makeToast(mContext, "不支持输入表情", Toast.LENGTH_SHORT).show();
            return "";
        } else if (invalidMatcher.find()) {
            UIAction.makeToast(mContext, "不支持输入特殊字符", Toast.LENGTH_SHORT).show();
            return "";
        }
        return null;
    }
